    ...
    bool intersect(const Ray &r, Hit &h, float tmin)
    {
        ...
        while (isInsideGird_(i_, j_, k_))
        {
            ...
            if (array_obj[i_][j_][k_].getNumObjects() != 0) //&& isInsideCell(i_, j_, k_, r.pointAtParameter(tintersect + t_current)))
            {
                ...
                /*if (array_obj[i_][j_][k_].getObject(0)->getMatrix() != NULL)
                {
                    Vec4f Origin_Os(r.getOrigin(), 1); // point
                    Vec4f Dir_Os(r.getDirection(), 0);
                    Vec3f Origin_Ob;
                    Vec3f Dir_Ob;
                    Matrix matrixInverse = *array_obj[i_][j_][k_].getObject(0)->getMatrix();
