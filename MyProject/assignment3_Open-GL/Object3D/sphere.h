...
public:
    ...
    bool intersecA(const Ray &r, Hit &h, float tmin)
    {
        // Algebraic
        Vec3f R0 = r.getOrigin() - center;
        float a = 1;
    ...
    {
        m->glSetMaterial();
        // int P_steps = 5, T_steps = 10;
        int P_steps = 15, T_steps = 30;
        const int array_num = 360 / T_steps + 1;
        Vec3f point[array_num], pre;
        for (int iPhi = -90 + P_steps; iPhi < 90; iPhi += P_steps)
        {
            for (int iTheta = 0; iTheta <= 360; iTheta += T_steps)
