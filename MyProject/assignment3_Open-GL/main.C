...
void RayCasting(SceneParser &parser)
{
    Vec3f Black(0, 0, 0);
...
void NormalVisualization(SceneParser &parser)
{
...
void DiffuseShading(SceneParser &parser)
{
    Image image(width, height);
    image.SetAllPixels(parser.getBackgroundColor());
    ...
    for (float i = 0; i < pixel; i++)
    {
        for (float j = 0; j < pixel; j++)
        {
            ...
            if (parser.getGroup()->intersect(parser.getCamera()->generateRay(p), h, parser.getCamera()->getTMin()))
            {
                ...
                Vec3f pcolor(0, 0, 0);
                Vec3f::Mult(am, am, h.getMaterial()->getDiffuseColor());
                ...
                for (int l = 0; l < lnum; l++)
                {
                    ...
                    parser.getLight(l)->getIllumination(h.getIntersectionPoint(), ldir, lcolor, distanceToLight);
                    // (const Vec3f &p, Vec3f &dir, Vec3f &col)
                    pcolor += h.getMaterial()->Shade(parser.getCamera()->generateRay(p), h, ldir, lcolor); // diffuse+ specular color
...
void renderFunction()
{
    printf("Having called renderFunction. Main.C will exec the render process.");
}
...
int main(int argc, char *argv[])
{
    ...
    RayCasting(parser);
    if (is_normal)
        NormalVisualization(parser);*/
    DiffuseShading(parser);

    if (gui)
    {
        glutInit(&argc, argv);
        SceneParser *pa = &parser;
        canvas.initialize(pa, renderFunction);
