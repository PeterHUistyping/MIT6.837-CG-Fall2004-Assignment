...
int main(int argc, char *argv[])
{
    ...
    RayTracer r_tracer(parser, max_bounces, cutoff_weight, shadows);
    ray_tracer = &r_tracer;
    if (isdepth)
        RayCasting();
    if (is_normal)
    ...
    {
        glutInit(&argc, argv);
        canvas.initialize(parser, DiffuseShading, traceRayFunction);
        //   void (*_renderFunction)(void),void (*_traceRayFunction)(float, float))
    }
}
